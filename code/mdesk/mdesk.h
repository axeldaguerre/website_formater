...
internal void MD_MsgListPush(Arena *arena, MD_MsgList *msgs, MD_Node *node, MD_MsgKind kind, String8 string);
internal void MD_MsgListPushF(Arena *arena, MD_MsgList *msgs, MD_Node *node, MD_MsgKind kind, char *fmt, ...);
internal void MD_MsgListConcatInPlace(MD_MsgList *dst, MD_MsgList *src);

////////////////////////////////
//~ rjf: Token Type Functions

internal MD_Token MD_TokenMake(Rng1U64 range, MD_TokenFlags flags);
internal B32 MD_TokenMatch(MD_Token a, MD_Token b);
internal String8List MD_StringListFromTokenFlags(Arena *arena, MD_TokenFlags flags);
internal void MD_TokenChunkListPush(Arena *arena, MD_TokenChunkList *list, U64 cap, MD_Token token);
internal MD_TokenArray MD_TokenArrayFromChunkList(Arena *arena, MD_TokenChunkList *chunks);
internal String8 MD_ContentStringFromTokenFlagsStr8(MD_TokenFlags flags, String8 string);

////////////////////////////////
//~ rjf: Node Type Functions

//- rjf: flag conversions
internal MD_NodeFlags MD_NodeFlagsFromTokenFlags(MD_TokenFlags flags);

//- rjf: nil
internal B32 MD_NodeIsNil(MD_Node *node);
#define MD_NodeSetNil(p) ((p) = &md_nil_node)

//- rjf: iteration
#define MD_EachNode(it, first) MD_Node *it = first; !MD_NodeIsNil(it); it = it->next
internal MD_NodeRec MD_NodeRecDepthFirst(MD_Node *node, MD_Node *subtree_root, MemberOffset child_off, MemberOffset sib_off);
...
//- rjf: tree building
internal MD_Node *MD_PushNode(Arena *arena, MD_NodeKind kind, MD_NodeFlags flags, String8 string, String8 raw_string, U64 src_offset);
internal void MD_NodePushChild(MD_Node *parent, MD_Node *node);
internal void MD_NodePushTag(MD_Node *parent, MD_Node *node);

//- rjf: tree introspection
internal MD_Node *  MD_NodeFromChainString(MD_Node *first, MD_Node *opl, String8 string, MatchFlags flags);
internal MD_Node *  MD_NodeFromChainIndex(MD_Node *first, MD_Node *opl, U64 index);
internal MD_Node *  MD_NodeFromChainFlags(MD_Node *first, MD_Node *opl, MD_NodeFlags flags);
internal U64        MD_IndexFromNode(MD_Node *node);
internal MD_Node *  MD_RootFromNode(MD_Node *node);
internal MD_Node *  MD_ChildFromString(MD_Node *node, String8 child_string, MatchFlags flags);
internal MD_Node *  MD_TagFromString(MD_Node *node, String8 tag_string, MatchFlags flags);
internal MD_Node *  MD_ChildFromIndex(MD_Node *node, U64 index);
internal MD_Node *  MD_TagFromIndex(MD_Node *node, U64 index);
internal MD_Node *  MD_TagArgFromIndex(MD_Node *node, String8 tag_string, MatchFlags flags, U64 index);
internal MD_Node *  MD_TagArgFromString(MD_Node *node, String8 tag_string, MatchFlags tag_str_flags, String8 arg_string, MatchFlags arg_str_flags);
internal B32        MD_NodeHasChild(MD_Node *node, String8 string, MatchFlags flags);
internal B32        MD_NodeHasTag(MD_Node *node, String8 string, MatchFlags flags);
internal U64        MD_ChildCountFromNode(MD_Node *node);
internal U64        MD_TagCountFromNode(MD_Node *node);

//- rjf: tree comparison
internal B32 MD_NodeDeepMatch(MD_Node *a, MD_Node *b, MatchFlags flags);
internal B32 MD_NodeMatch(MD_Node *a, MD_Node *b, MatchFlags flags);
...
internal MD_ParseResult MD_ParseFromTextTokens(Arena *arena, String8 filename, String8 text, MD_TokenArray tokens);

////////////////////////////////
//~ rjf: Tree -> Text Functions

internal String8List MD_DebugStringListFromTree(Arena *arena, MD_Node *root);
